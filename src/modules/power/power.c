...
static int power_chip_init(uint8_t *part_info)
{
    uint8_t part = 0U;
    ...
    if (bq25756_probe(&charger, &part) != BQ25756_OK) {
        (void)console_print("pwr: probe failed, part 0x%x\r\n", part);
        return -1;
    }
    if (part_info != 0)
        *part_info = part;
    ...
    if (bq25756_configure_software_host(&charger) != BQ25756_OK ||
        ...
        (void)console_puts("pwr: host/adc setup failed");
        return -1;
    }

    (void)console_print("pwr: BQ25756 ready, part 0x%x, I2C1 PB8/PB9 100 kHz, charge off\r\n", part);
    return 0;
}
...
static void power_thread_entry(ULONG arg)
{
    ...
    while (power_chip_init(&part_info) != 0) {
        (void)console_puts("pwr: init failed, retry");
    ...
    for (;;) {
        (void)tx_mutex_get(&power_lock, TX_WAIT_FOREVER);
        if (refresh_locked() != 0)
            (void)console_puts("pwr: read failed");
        (void)tx_mutex_put(&power_lock);
