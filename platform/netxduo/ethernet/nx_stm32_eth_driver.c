...
static VOID  _nx_driver_enable(NX_IP_DRIVER *driver_req_ptr)
{
  ...
  do {
    ...
    if (PHYLinkState > ETH_PHY_STATUS_LINK_DOWN)
      break;
    tx_thread_sleep(100);
  } while ((HAL_GetTick() - tickstart) < PHY_LINK_TIMEOUT);

  /* No cable: still start the MAC at 100M/FD so DHCP/static IP can come up later. */
  if (PHYLinkState <= ETH_PHY_STATUS_LINK_DOWN)
  {
  ...
  MACConf.Speed = speed;
#if defined(ETH_DMASBMR_BLEN4)
  MACConf.PortSelect = (speed == ETH_SPEED_1000M) ? DISABLE : ENABLE;
#endif
...
static UINT  _nx_driver_hardware_initialize(NX_IP_DRIVER *driver_req_ptr)
{
  ...
  nx_eth_init();
  if (eth_handle.gState != HAL_ETH_STATE_READY)
    return NX_DRIVER_ERROR;
